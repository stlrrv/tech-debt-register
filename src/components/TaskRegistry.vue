  ...
  <div style="min-height: 100vh; background: #f9fafb; padding: 1.5rem 1rem">
    <div style="max-width: 960px; margin: 0 auto">
      ...
      <div v-if="!isReadOnly" class="card" style="margin-bottom: 1.5rem">
        ...
        <h3 class="group-label" style="margin-top: 1.5rem">
          Оценки приоритезации
        </h3>
        <div class="form-grid">
          <div class="field">
...
const quadrantDefs = [
  {
    key: "first",
    label: "Делаем первыми",
    desc: "Высокая ценность · Малые усилия",
    bg: "#f0fdf4",
    border: "#86efac",
    color: "#166534",
  },
  ...
  },
  {
    key: "residual",
    label: "По остаточному",
    desc: "Низкая ценность · Малые усилия",
    bg: "#f9fafb",
    border: "#d1d5db",
    color: "#374151",
  },
...
function calcValue(task) {
  return (
    Number(task.processScore) * 3 +
    Number(task.delayScore) * 2 +
    Number(task.impactScore) +
    Number(task.deadlineScore)
  );
}
...
function getRecommendation(q) {
  return (
    {
      "Делаем первыми": "Берём в ближайший спринт",
      Планируем: "Декомпозируем и планируем",
      "По остаточному": "Берём когда нет важных задач",
      "Пока не делаем": "Отказываемся или переформулируем",
    }[q] || ""
  );
}
...
function updatePreview() {
  const v =
    Number(form.processScore) * 3 +
    Number(form.delayScore) * 2 +
    Number(form.impactScore) +
    Number(form.deadlineScore);
...
function sizeBadgeStyle(size) {
  const m = {
    ...
    M: "background:#fef9c3;color:#854d0e;border:1px solid #fde047;",
    L: "background:#fee2e2;color:#991b1b;border:1px solid #fca5a5;",
...
function statusBadgeStyle(s) {
  const m = {
    new: "background:#f8fafc;color:#64748b;border:1px solid #cbd5e1;",
    candidate: "background:#eff6ff;color:#2563eb;border:1px solid #93c5fd;",
    planned: "background:#f0fdf4;color:#16a34a;border:1px solid #86efac;",
    in_progress: "background:#fffbeb;color:#d97706;border:1px solid #fcd34d;",
    done: "background:#dcfce7;color:#15803d;border:1px solid #4ade80;",
    deferred: "background:#fafafa;color:#a3a3a3;border:1px solid #e5e5e5;",
  };
  return m[s] || "";
}
...
function dirLabel(d) {
  return { web: "Web", onec: "1С", qa: "QA", common: "Общее" }[d] || d;
}
...
function startEdit(task) {
  ...
  Object.assign(form, {
    ...
    size: task.size,
    processScore: String(task.processScore),
...
function saveToLocalStorage() {
  localStorage.setItem("taskRegistry", JSON.stringify(tasks.value));
}
